...
const Header = () => {
  ...
  return (
    <header style={{ display: 'flex', flexDirection: 'row', maxWidth: 960, margin: '0 auto 1.45rem', flexWrap: 'wrap' }}>
      <div style={{ display: 'flex', flexDirection: 'row', flex: 2, padding: '1.45rem 1.0875rem' }}>
        <div style={{ width: 70 }}>
          <Img fixed={data.logoImage.childImageSharp.fixed} />
        </div>
        <h1 style={{ flex: 'auto', margin: 0, fontSize: '1.9rem', alignItems: 'center', display: 'flex', flexWrap: 'nowrap', whiteSpace: 'nowrap' }}>{data.site.siteMetadata.title}</h1>
      </div>
      <div style={{display: 'flex', flex: 1, justifyContent: 'flex-end', alignItems: 'center', flexDirection: 'row', marginRight: '20px'}}>
        ...
        <a href="https://discord.gg/J9epcs" target="_blank"><Img style={{marginLeft: 10 }} fixed={data.discordImage.childImageSharp.fixed} /></a>
      </div>
    </header>
  )
}
